...
void main() {
  ...
  c += (n - 0.5) * uNoiseAmount * min(intensity * 5.0, 1.0);
  
  // Calculate radial distance to the canvas center and apply dynamic circular edge falloff
  vec2 uv = gl_FragCoord.xy / uResolution.xy;
  float radialDist = length(uv - 0.5);
  float borderFade = smoothstep(0.5, 0.33, radialDist);
  
  gl_FragColor = vec4(c, intensity * uOpacity * borderFade);
}
`;
